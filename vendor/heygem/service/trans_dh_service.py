...
# Internal filename: /code/service/trans_dh_service.py
# Bytecode version: 3.8.0rc1+ (3413)
# Source timestamp: 2024-04-01 02:48:56 UTC (1711939736)
...
def drivered_video(code, drivered_queue, drivered_path, audio_wenet_feature, batch_size, wh=0):
    try:
        ...
        drivered_queue.put([False, '[{}]任务视频驱动队列异常，异常信息:[{}]'.format(code, e.__str__()), code])
                    ...
                    while cap.isOpened():
                        ...
                        if ret:
                            ...
                            if current_idx % batch_size == 0:
                                ...
                                wenet_feature_list = []
                            if current_idx == len(audio_wenet_feature):
                                if len(drivered_list) > 0 and len(wenet_feature_list) > 0:
                                    drivered_queue.put([drivered_list, wenet_feature_list, code, wh, current_idx], block=True, timeout=60)
                                    drivered_list = []
                                    wenet_feature_list = []
                                logger.info('append imgs over')
...
class TransDhTask(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        multiprocessing.Process(target=audio_transfer, args=(self.drivered_queue, self.output_imgs_queue, self.batch_size)).start()
        self.init_wh_queue = multiprocessing.Queue(2)
        self.init_wh_queue_output = multiprocessing.Queue(2)
        multiprocessing.Process(target=init_wh_process, args=(self.init_wh_queue, self.init_wh_queue_output)).start()
    ...
    def work(self, audio_url, video_url, code, watermark_switch, digital_auth, chaofen, pn):
        ...
        try:
            ...
            if not os.path.exists(_video_url) or not os.path.exists(_audio_url):
            ...
            if pn == 1:
                process_list.append(Process(target=drivered_video_pn, args=(code, self.drivered_queue, _tmp_video_path, audio_wenet_feature, self.batch_size, wh)))
            else:
                process_list.append(Process(target=drivered_video, args=(code, self.drivered_queue, _tmp_video_path, audio_wenet_feature, self.batch_size, wh)))
            if chaofen == 1 and GlobalConfig.instance().chaofen_after == '1':
                process_list.append(Process(target=write_video_chaofen, args=(self.output_imgs_queue, GlobalConfig.instance().temp_dir, GlobalConfig.instance().result_dir, code, _tmp_audio_path, self.result_queue, width, height, fps, watermark_switch, digital_auth)))
            else:
                process_list.append(Process(target=write_video, args=(self.output_imgs_queue, GlobalConfig.instance().temp_dir, GlobalConfig.instance().result_dir, code, _tmp_audio_path, self.result_queue, width, height, fps, watermark_switch, digital_auth)))
            [p.start() for p in process_list]
            [p.join() for p in process_list]
            try:
